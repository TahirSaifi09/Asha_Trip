/** @format */
import { FaCircle } from "react-icons/fa";

export default function Blog() {
  return (
    <div className="m-4 w-8/12 flex-wrap max-lg:w-full">
      {/* we care about your privacy */}
      <div className="p-4 shadow-xl text-sm font-medium rounded-2xl bg-white">
        <div>
          <h1 className="text-lg font-semibold py-2">
            We care about your privacy{" "}
          </h1>
          <hr className="w-1/2 mt-2 mb-2"></hr>
        </div>
        <div>
          <p className="text-sm text-gray-400">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula tincidunt
            fusce. Egestas commodo neque mattis enim magnis mauris massa. Donec
            id eu sapien pulvinar malesuada tempor sociis quis. Porta turpis
            tellus pellentesque enim elementum. Netus in facilisis pharetra
            pellentesque eu eget. Egestas euismod ac vitae nisl quam sed urna.
            Pretium amet.
          </p>
        </div>
      </div>
      {/* our privacy policy in brief */}
      <div className="p-4 mt-6 shadow-xl border font-medium rounded-2xl bg-white ">
        <div>
          <h1 className="text-lg font-semibold py-2">
            Our Privacy Policy in brief
          </h1>
          <hr className="w-1/2 mt-2 mb-2"></hr>
        </div>
        <div>
          <p className="text-sm text-gray-400">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula tincidunt
            fusce. Egestas commodo neque mattis enim magnis mauris massa. Donec
            id eu sapien pulvinar malesuada tempor sociis quis. Porta turpis
            tellus pellentesque enim elementum. Netus in facilisis pharetra
            pellentesque eu eget. Egestas euismod ac vitae nisl quam sed urna.
            Pretium amet.
          </p>
        </div>
        <div className=" text-gray-400">
          <p className="text-sm mt-4 mb-4 text-gray-800">
            Our Privacy Policy in brief
          </p>

          <div className="ml-4">
            <div className="flex gap-4">
              <FaCircle size={6} className="mt-2" />
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur. Maecenas lobortis
                gravida lobortis nulla nibh. Blandit adipiscing.
              </p>
            </div>
            <div className="flex gap-4">
              <FaCircle size={6} className="mt-2" />
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur. Maecenas lobortis
                gravida lobortis nulla nibh. Blandit adipiscing.
              </p>
            </div>
            <div className="flex gap-4">
              <FaCircle size={6} className="mt-2" />
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur. Maecenas lobortis
                gravida lobortis nulla nibh. Blandit adipiscing.
              </p>
            </div>
            <div className="flex gap-4">
              <FaCircle size={6} className="mt-2" />
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur. Maecenas lobortis
                gravida lobortis nulla nibh. Blandit adipiscing.
              </p>
            </div>
          </div>
        </div>
        <div className="text-gray-400">
          <p className="text-sm mt-4 mb-4 text-gray-800">
            How we use your Personal Info
          </p>
          <p className="text-sm mb-2">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula{" "}
          </p>
          <div className="ml-4">
            <div className="flex gap-4">
              <FaCircle size={6} className="mt-2" />
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur. Maecenas lobortis
                gravida lobortis nulla nibh. Blandit adipiscing.
              </p>
            </div>
            <div className="flex gap-4">
              <FaCircle size={6} className="mt-2" />
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur. Maecenas lobortis
                gravida lobortis nulla nibh. Blandit adipiscing.
              </p>
            </div>
            <div className="flex gap-4">
              <FaCircle size={6} className="mt-2" />
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur. Maecenas lobortis
                gravida lobortis nulla nibh. Blandit adipiscing.
              </p>
            </div>
            <div className="flex gap-4">
              <FaCircle size={6} className="mt-2" />
              <p className="text-sm">
                Lorem ipsum dolor sit amet consectetur. Maecenas lobortis
                gravida lobortis nulla nibh. Blandit adipiscing.
              </p>
            </div>
          </div>
        </div>
        <div>
          <div>
            <h1 className="text-sm  text-gray-800 mt-4 mb-4">Cookies</h1>
          </div>
          <div>
            <p className="text-sm text-gray-400">
              Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
              lobortis nulla nibh. Blandit adipiscing gravida suspendisse
              lacinia. Eleifend sed turpis aliquet non condimentum cursus ligula
              tincidunt fusce. Egestas commodo neque mattis enim magnis mauris
              massa. Donec id eu sapien pulvinar malesuada tempor sociis quis.
              Porta turpis tellus pellentesque enim elementum. Netus in
              facilisis pharetra pellentesque eu eget. Egestas euismod ac vitae
              nisl quam sed urna. Pretium amet.
            </p>
          </div>
        </div>
      </div>

      {/* our privacy policy in brief */}
      <div className="mt-4 p-4 shadow-xl border rounded-2xl bg-white">
        <div>
          <h1 className="text-lg font-semibold">
            We Our Privacy Policy in brief{" "}
          </h1>
          <hr className="w-1/2 mt-2 mb-2"></hr>
        </div>
        <div>
          <p className="text-sm text-gray-400">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula tincidunt
            fusce. Egestas commodo neque mattis enim magnis mauris massa. Donec
            id eu sapien pulvinar malesuada tempor sociis quis. Porta turpis
            tellus pellentesque enim elementum. Netus in facilisis pharetra
            pellentesque eu eget. Egestas euismod ac vitae nisl quam sed urna.
            Pretium amet.
          </p>
        </div>
        <div className="text-gray-400 ml-5">
          <div className="flex gap-4">
            <FaCircle size={18} className="mt-2" />
            <p>
              Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
              lobortis nulla nibh. Blandit adipiscing gravida suspendisse
              lacinia. Eleifend sed turpis aliquet non condimentum cursus ligula
              tincidunt fusce. Egestas commodo neque mattis enim magnis mauris
              massa. Donec id eu sapien pulvinar malesuada tempor sociis quis.
            </p>
          </div>
          <div className="flex gap-4">
            <FaCircle size={18} className="mt-2" />
            <p>
              Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
              lobortis nulla nibh. Blandit adipiscing gravida suspendisse
              lacinia. Eleifend sed turpis aliquet non condimentum cursus ligula
              tincidunt fusce. Egestas commodo neque mattis enim magnis mauris
              massa. Donec id sapien pulvinar malesuada tempor sociis quis.
            </p>
          </div>
        </div>
      </div>

      {/* providing your personal information */}
      <div className="p-4 shadow-xl border rounded-2xl bg-white mt-4">
        <div>
          <h1 className="text-lg font-semibold">
            Providing your personal information
          </h1>
          <hr className="w-1/2 mt-2 mb-2"></hr>
        </div>
        <div>
          <p className="text-sm text-gray-400">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula tincidunt
            fusce. Egestas commodo neque mattis enim magnis mauris massa. Donec
            id eu sapien pulvinar malesuada tempor sociis quis. Porta turpis
            tellus pellentesque enim elementum. Netus in facilisis pharetra
            pellentesque eu eget. Egestas euismod ac vitae nisl quam sed urna.
            Pretium amet.
          </p>
        </div>
      </div>
      {/* third party websites */}
      <div className=" p-4 shadow-xl border rounded-2xl bg-white mt-4">
        <div>
          <h1 className="text-lg font-semibold">Third party websites</h1>
          <hr className="w-1/2 mt-2 mb-2"></hr>
        </div>
        <div>
          <p className="text-sm text-gray-400">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula tincidunt
            fusce. Egestas commodo neque mattis enim magnis mauris massa. Donec
            id eu sapien pulvinar malesuada tempor sociis quis. Porta turpis
            tellus pellentesque enim elementum. Netus in facilisis pharetra
            pellentesque eu eget. Egestas euismod ac vitae nisl quam sed urna.
            Pretium amet.
          </p>
        </div>
      </div>

      {/* updates */}
      <div className="p-4 shadow-xl border rounded-2xl bg-white mt-4">
        <div>
          <h1 className="text-lg font-semibold">Updates</h1>
          <hr className="w-1/2 mt-2 mb-2"></hr>
        </div>
        <div>
          <p className="text-sm text-gray-400">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula tincidunt
            fusce. Egestas commodo neque mattis enim magnis mauris massa. Donec
            id eu sapien pulvinar malesuada tempor sociis quis. Porta turpis
            tellus pellentesque enim elementum. Netus in facilisis pharetra
            pellentesque eu eget. Egestas euismod ac vitae nisl quam sed urna.
            Pretium amet.
          </p>
        </div>
      </div>
      {/* Legal Basis for using your personal information */}
      <div className="p-4 shadow-xl border rounded-2xl bg-white mt-4">
        <div>
          <h1 className="text-lg font-semibold ">
            Legal Basis for using your personal information
          </h1>
          <hr className="w-1/2 mt-2 mb-2"></hr>
        </div>
        <div>
          <p className="text-sm text-gray-400">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula tincidunt
            fusce. Egestas commodo neque mattis enim magnis mauris massa. Donec
            id eu sapien pulvinar malesuada tempor sociis quis. Porta turpis
            tellus pellentesque enim elementum. Netus in facilisis pharetra
            pellentesque eu eget. Egestas euismod ac vitae nisl quam sed urna.
            Pretium amet.
          </p>
        </div>

        <div className="ml-4 mt-4 text-gray-400">
          <div className="flex gap-4">
            <FaCircle size={6} className="mt-2" />
            <p className="text-sm">
              Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
              lobortis nulla nibh. Blandit adipiscing.
            </p>
          </div>
          <div className="flex gap-4">
            <FaCircle size={6} className="mt-2" />
            <p className="text-sm">
              Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
              lobortis nulla nibh. Blandit adipiscing.
            </p>
          </div>
          <div className="flex gap-4">
            <FaCircle size={6} className="mt-2" />
            <p className="text-sm">
              Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
              lobortis nulla nibh. Blandit adipiscing.
            </p>
          </div>
          <div className="flex gap-4">
            <FaCircle size={6} className="mt-2" />
            <p className="text-sm">
              Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
              lobortis nulla nibh. Blandit adipiscing.
            </p>
          </div>
        </div>
      </div>

      {/* Cookies */}
      <div className="p-4 shadow-xl border rounded-2xl bg-white mt-4">
        <div>
          <h1 className="text-lg font-semibold">Cookies </h1>
          <hr className="w-1/2 mt-2 mb-2"></hr>
        </div>
        <div>
          <p className="text-sm text-gray-400">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula tincidunt
            fusce. Egestas commodo neque mattis enim magnis mauris massa. Donec
            id eu sapien pulvinar malesuada tempor sociis quis. Porta turpis
            tellus pellentesque enim elementum. Netus in facilisis pharetra
            pellentesque eu eget. Egestas euismod ac vitae nisl quam sed urna.
            Pretium amet.
          </p>
        </div>
      </div>
      {/* How to Contacct Us */}
      <div className="p-4 shadow-xl border rounded-2xl bg-white mt-4 ">
        <div>
          <h1 className="text-lg font-semibold">How to Contacct Us </h1>
          <hr className="w-1/2 mt-2 mb-2"></hr>
        </div>
        <div>
          <p className="text-sm text-gray-400">
            Lorem ipsum dolor sit amet consectetur. Maecenas lobortis gravida
            lobortis nulla nibh. Blandit adipiscing gravida suspendisse lacinia.
            Eleifend sed turpis aliquet non condimentum cursus ligula tincidunt
            fusce. Egestas commodo neque mattis enim magnis mauris massa. Donec
            id eu sapien pulvinar malesuada tempor sociis quis. Porta turpis
            tellus pellentesque enim elementum. Netus in facilisis pharetra
            pellentesque eu eget. Egestas euismod ac vitae nisl quam sed urna.
            Pretium amet.
          </p>
        </div>
      </div>
    </div>
  );
}
